...
class VictimParameters(object):
	...
	def default_request_handler(self, request):
		'''Default request handler, just checks if in_request string is contained in the request.
	...
	def proc_in_request(self, request):
		'''Process request, send it to custom handler if set, otherwise use default.'''
